'''
Daily VBD NC 자료를 읽어서 Monthly VBD NC 생성
2021.09.10
Auth: Hokun Jeon
...
path_out_NC='D:/20_Product/VBD/NC/ScatterMonthly'
path_Lib='D:/programming/SatModel/Lib'
sys.path.append(path_Lib)

def read_oneday_NC(path_file):
    # -*- coding: utf-8 -*-
    # path_file='D:/20_Product/VBD/NC/MonthlyScatter/2017/ShipScatter_Donghae_2017-07-01.nc'
    import Map
...
def make_monthly_NC(lons, lats, Date_Mscans, year, month, area):
    import netCDF4 as nc
    dir = path_out_NC + '/' + year
    os.makedirs(dir, exist_ok=True)
    fn = dir + '/ShipScatter' + '_' + area + '_' + year + '-' + month + '.nc'
    ...
    ds.title = 'Ship Scatter ' + area + '-' + year + '-' + month
    ...
    scan = ds.createVariable('Date_Mscan', str, ('Date_Mscan'))
...
for year in years:
    ...
    # area='PIF'
    for month in months:
        month_file_list = file_list[np.array([month==name.split('-')[1] for name in file_list])]
        if len(month_file_list)!=0:
            for area in areas:
                print(year + ' ' + month + ' ' + area)
                area_file_list = month_file_list[np.array([area in name for name in month_file_list])]
                lons=np.array([])
                lats=np.array([])
                Date_Mscans=np.array([])
                for file_name in area_file_list:
                    # file_name='ShipScatter_PIF_2016-08-25.nc'
                    path_file = path_year +'/'+ file_name
                    lon, lat, Date_Mscan=read_oneday_NC(path_file)
                    lons = np.concatenate([lons, lon])
                    lats = np.concatenate([lats, lat])
                    Date_Mscans = np.concatenate([Date_Mscans, Date_Mscan])
                make_monthly_NC(lons, lats, Date_Mscans, year, month, area)