...
def RFC_evaluation(train_input, train_target):
    # 교차검증 수행
    from sklearn.model_selection import cross_validate
    from sklearn.ensemble import RandomForestClassifier
    rf = RandomForestClassifier(n_jobs=-1, random_state=42)
    scores = cross_validate(rf, train_input, train_target, return_train_score=True, n_jobs=-1)
    print('CV Train Score: %.4f CV Test Score: %.4f ' %\
    ...
    print(np.round(np.array(rf.feature_importances_),4))
    ...
    return rf, rf.feature_importances_, scores


def RFC_fit(train_input, train_target):
    # 교차검증 수행
    from sklearn.model_selection import cross_validate
    from sklearn.ensemble import RandomForestClassifier
    rf = RandomForestClassifier(n_jobs=-1, random_state=42)
