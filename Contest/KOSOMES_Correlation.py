...
path_source_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data'
file_name='KOSOMES_0p05.nc'
path_corr_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/corr'
path_scatter_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/scatter'
path_hist_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/hist'
path_data_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/data'
...
for mm in range(len(month_str)):
    # mm=0
    date_M = time[np.array(['2020-%s-' % month_str[mm] in name for name in time])]
    ...
    for ii in range(len(date_M)):
        ...
        now_vbd=vbd[cond_date,:,:].reshape(grid_y,grid_x)
        ...
        p_chl.extend(chl[cond_date,cond_psn].flatten())
        p_fe.extend(fe[cond_date,cond_psn].flatten())
        p_no3.extend(no3[cond_date,cond_psn].flatten())
        p_nppy.extend(nppy[cond_date,cond_psn].flatten())
        p_o2.extend(o2[cond_date,cond_psn].flatten())
        p_ph.extend(ph[cond_date,cond_psn].flatten())
        p_phyc.extend(phyc[cond_date,cond_psn].flatten())
        p_si.extend(si[cond_date,cond_psn].flatten())
        p_spco2.extend(spco2[cond_date,cond_psn].flatten())
        p_sst.extend(sst[cond_date,cond_psn].flatten())
        p_sla.extend(sla[cond_date,cond_psn].flatten())
    ...
    for ii in range(4,len(DF.columns)):
        ...
        plt.grid()
        path_hist_dir = 'D:/30_Conference/2021-11_KOSOMES(PUS)/data/hist/thred' + str(thred_ships)
        os.makedirs(path_hist_dir, exist_ok=True)
    ...
    path_corr_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/corr/thred'+str(thred_ships)
    ...
    plt.xticks(ticks=np.arange(0.5,len(corr.keys())+0.5,1),labels=corr.keys())
    plt.yticks(ticks=np.arange(0.5,len(corr.keys())+0.5,1),labels=corr.keys())
    plt.colorbar()
    plt.tight_layout()
    ...
    ## Correlation abs
    plt.figure(4)
    plt.pcolor(np.abs(np.array(corr)), vmin=0, vmax=1, cmap='jet')
    plt.xticks(ticks=np.arange(0.5,len(corr.keys())+0.5,1),labels=corr.keys())
    plt.yticks(ticks=np.arange(0.5,len(corr.keys())+0.5,1),labels=corr.keys())
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(path_corr_dir+'/corr_abs'+date_M[0][:7]+'.png')
    plt.clf()
    #
    #
    ## Scatter
    ...
    for ii in range(3,len(DF.columns)):
        ...
        plt.ylabel(DF_keys[ii])
        plt.grid()
        path_scatter_dir = 'D:/30_Conference/2021-11_KOSOMES(PUS)/data/scatter/thred' + str(thred_ships)
    ...
    ## DF 저장
    path_DF_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/DF/thred'+str(thred_ships)
