...
path_DF_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/DF/thred0'
...
PCA_resp=pd.DataFrame(columns=cols)
...
for ii in range(len(DF_names)):
    ...
    train_input, test_input, train_target, test_target\
        =train_test_split(DF.drop(columns=['vbd','lon','lat']).astype(float), DF['vbd'].astype(int), test_size=.3, random_state=42)

    train_input, test_input, train_target, test_target\
        =train_test_split(DF[['no3','o2','ph','sla','sst']].astype(float), DF['vbd'].astype(int), test_size=.3, random_state=42)

    regr, prediction=RF.RF(train_input, test_input, train_target, test_target,
       max_depth=30, random_state=531, n_estimators=150)
    ...
    train_input, test_input, train_target, test_target\
        =train_test_split(DF[['bath','chl']], DF['vbd'], test_size=.3, random_state=42)
...
perform_dir='D:/30_Conference/2021-11_KOSOMES(PUS)/data/perform'
