...
path_VBD = 'E:/20_Product/VBD/NC/DensityDaily/2020/ShipDensity_Donghae_2020-01-01-155807_0p05.nc'
...
import pandas as pd
file_list=recursive_file('E:/20_Product/VBD/NC/DensityDaily/2020','*0p05.nc')
...
### VBD의 날짜
# ii=0
date_list=np.array([file_list[ii].split('\\')[-1].split('_')[2][:10] for ii in range(len(file_list))])
datetime_list=np.array([file_list[ii].split('\\')[-1].split('_')[2] for ii in range(len(file_list))])
path_out_dir='E:/20_Product/FishingBoatConcentration/data'
datetime_DF=pd.DataFrame(datetime_list,columns=['UTC']).to_csv(path_out_dir+'/VBDdatetime.csv', index=False)
# ### 정상적인 날짜
...
Data_date=np.array([Data_BIO['time'][ii].strftime(format='%Y-%m-%d') for ii in range(len(Data_BIO['time']))])

## 동해좌표 읽기
import Lib.Map as Map
coord = Map.sector()['Donghae']

## 동일날짜에 2,3회씩 VBD 반복되므로, 반복을 고려한 날짜목록 생성
VBD_env_match_idx=np.zeros((len(date_list),1))
VBD_env_match_idx[:]=np.nan
for ii in range(len(date_list)):
    VBD_env_match_idx[ii]=float(np.where(date_list[ii]==Data_date)[0])
VBD_env_match_idx=VBD_env_match_idx.T.astype(int)


## 데이터 읽기
n_CHL=Data_BIO['chl'][VBD_env_match_idx,:,:][0,:,:,:]
n_FE=Data_BIO['fe'][VBD_env_match_idx,:,:][0,:,:,:]
n_NO3=Data_BIO['no3'][VBD_env_match_idx,:,:][0,:,:,:]
n_NPPY=Data_BIO['nppy'][VBD_env_match_idx,:,:][0,:,:,:]
n_O2=Data_BIO['o2'][VBD_env_match_idx,:,:][0,:,:,:]
n_PH=Data_BIO['ph'][VBD_env_match_idx,:,:][0,:,:,:]
n_PHYC=Data_BIO['phyc'][VBD_env_match_idx,:,:][0,:,:,:]
n_PO4=Data_BIO['po4'][VBD_env_match_idx,:,:][0,:,:,:]
n_SI=Data_BIO['si'][VBD_env_match_idx,:,:][0,:,:,:]
n_SPCO2=Data_BIO['spco2'][VBD_env_match_idx,:,:][0,:,:,:]
...
for ii in tqdm(range(N)):
    ...
    n2_CHL[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_CHL)
    n2_FE[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_FE)
    n2_NO3[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_NO3)
    n2_NPPY[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_NPPY)
    n2_O2[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_O2)
    n2_PH[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_PH)
    n2_PHYC[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_PHYC)
    n2_PO4[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_PO4)
    n2_SI[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_BIO_lon, n_BIO_lat, ns_SI)
    n2_SPCO2[ii, :, :] = matchedArray(mesh_lon, mesh_lat, n_BIO_lon, n_BIO_lat, ns_SPCO2)
    n2_SST[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_SST_lon, n_SST_lat, ns_SST)
    n2_SLA[ii,:,:]=matchedArray(mesh_lon,mesh_lat,n_SLA_lon, n_SLA_lat, ns_SLA)


'''
################################
FBC 데이터셋을 하나의 NC로 만들기
Fishing Boat Concentration
################################
'''

import netCDF4 as nc

path_dir_sub = 'E:/20_Product/FishingBoatConcentration/data'
