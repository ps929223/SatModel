...
get_ptspattern <- function(path_file){
  ...
  # path_file="E:/20_Product/CHL_DM/CHL_DM9/2018/03/GOCI1_DM9_CHL_2018-03-25_y86x88.nc"
  ...
  par(mfrow=c(2,2), cex.axis=2, cex.lab=2, cex.main=3)
  
  # cond_target=((lm==0)&(noa==0))&(!is.na(chl)) # 관측영역
  
  cond_target=((lm==0)&(noa==0))&(is.na(chl)) # 결측영역만
  
  plot(lon[cond_target],lat[cond_target], pch='*', main='Black: Cloud',
       xlab='Lon', ylab='Lat')
...
## DM1 기준 파일 목록
DM1_list = file_list.DM(path_source_dir='E:/20_Product/CHL_DM/CHL_DM1',pattern='.nc')
date_list=c()


## 날짜 목록 추출
# ii = 1
for (ii in 1:length(DM1_list)){
